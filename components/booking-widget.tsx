...
import { BOOKING_HREF, IS_EXTERNAL_BOOKING } from "@/lib/brand";
...
export function BookingWidget() {
  ...
  // One-way: placeholder slots → contact form (never back to #booking).
  // When Calendly is configured, open it directly instead.
  const ctaHref = IS_EXTERNAL_BOOKING
    ? BOOKING_HREF
    : buildHashHref("contact", {
        intent: "booking",
        industry,
  ...
  return (
    <div className="glass-card card-compact h-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs uppercase tracking-wider text-leaf font-medium bg-leaf/10 px-2 py-0.5 rounded-full border border-leaf/20">
          {IS_EXTERNAL_BOOKING ? "Calendly" : "Placeholder"}
        </span>
      </div>
      ...
      <CtaButton href={ctaHref} className="w-full" size="sm">
        {IS_EXTERNAL_BOOKING ? "Open Calendly" : "Continue to contact form"}
      </CtaButton>

      <p className="text-xs text-muted-light mt-3 leading-relaxed">
        {IS_EXTERNAL_BOOKING
          ? "Opens Calendly in a new tab to pick a live time."
          : "Calendly placeholder — pick a demo slot, then confirm on the contact form (no loop back)."}
      </p>
    </div>
  );
}
